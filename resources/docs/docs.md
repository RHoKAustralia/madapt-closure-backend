...
The `home-routes` handler in the `madapt-data-service.routes.home` namespace
...
The `home-page` function will in turn call the `madapt-data-service.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `madapt-data-service.handler` namespace:
...
Request middleware functions are located under the `madapt-data-service.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `madapt-data-service.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `madapt-data-service.db.core` in some other namespace.
